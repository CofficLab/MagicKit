    ...
    VStack(spacing: 20) {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 15) {
                DemoSection(title: "连接测试", icon: "📡") {
                    PingTestRow("google.com")
                    PingTestRow("baidu.com")
                    PingTestRow("github.com")
                    
                    DemoButton("详细Ping测试", action: {
                        do {
                ...
                DemoSection(title: "IP信息", icon: "🏠") {
                    IPInfoRow("本机IP", ShellNetwork.getLocalIPs().first ?? "未知")
                    
                    DemoButton("获取公网IP", action: {
                        let publicIP = ShellNetwork.getPublicIP()
                        print("公网IP: \(publicIP)")
                    })
                    
                    DemoButton("所有本机IP", action: {
                        let ips = ShellNetwork.getLocalIPs()
                        print("本机IP列表: \(ips)")
                    })
                }
                
                DemoSection(title: "端口测试", icon: "🚪") {
                    PortTestRow("google.com", 80)
                    PortTestRow("google.com", 443)
                    PortTestRow("github.com", 22)
                    PortTestRow("localhost", 3000)
                }
                
                DemoSection(title: "HTTP测试", icon: "🌍") {
                    HTTPStatusRow("https://www.google.com")
                    HTTPStatusRow("https://www.github.com")
                    HTTPStatusRow("https://httpstat.us/404")
                    
                    DemoButton("获取HTTP头", action: {
                        do {
                            let headers = try ShellNetwork.getHeaders("https://www.google.com")
                            print("HTTP头信息:\n\(headers)")
                        } catch {
                            print("获取HTTP头失败: \(error)")
                        }
                    })
                }
                
                DemoSection(title: "网络信息", icon: "ℹ️") {
                    DemoButton("网络接口状态", action: {
                        let status = ShellNetwork.getNetworkStatus()
                        print("网络接口状态:\n\(status)")
                    })
                    
                    DemoButton("路由表", action: {
                        let routes = ShellNetwork.getRoutes()
                        let lines = routes.components(separatedBy: .newlines)
                        print("路由表（前10行）:\n\(lines.prefix(10).joined(separator: "\n"))")
                    })
                    
                    DemoButton("WiFi信息", action: {
                        let wifi = ShellNetwork.getWiFiInfo()
                        print("WiFi信息:\n\(wifi)")
                    })
                }
                
                DemoSection(title: "DNS和路由", icon: "🔍") {
                    DemoButton("DNS查询", action: {
                        do {
                ...
                DemoSection(title: "速度测试", icon: "⚡") {
                    DemoButton("网络速度测试", action: {
