...
extension BackgroundGroup: View {}

// 使用示例
struct ExampleView: View {
    var body: some View {
        VStack {
            BackgroundGroup(for: "green2blue_tl2br")
            BackgroundGroup(for: "invalidGradient") // 将使用默认的 .blue2cyan
            BackgroundGroup(for: .red2yellow_tl2br) // 仍然可以使用枚举
        }
    }
}
        ...
        ScrollView {
            HStack {
                ForEach(1 ... 10, id: \.self) { i in
                    ...
                    VStack {
                        ForEach(BackgroundGroup.GradientName.allCases, id: \.self) { gradientName in
                            ZStack {
                                BackgroundGroup(for: gradientName).opacity(opacity)
                                VStack {
